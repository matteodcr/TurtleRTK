...
export default observer(function PeripheralList({navigation}: PeripheralListProps) {

  const store = useStoreContext();
  
  const peripheralsArray = Array.from(store.bluetoothManager.peripherals).map((item, index) => {
    return {
      ...item,
  ...
  return (
    <FlashList
      ...
      renderItem={({item}) => (<PeripheralListItem device={item} navigation={navigation}/>)}
